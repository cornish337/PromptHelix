...
def setup_logging():
    ...
    if 'console' in config_to_use['handlers']: # Ensure console handler exists before configuring
    ...
    if enable_file_logging:
        ...
        print(f"Logging Config: Standard file logging enabled. Path: {log_file_path}, Level: {LOG_LEVEL}")
    else:
        print(f"Logging Config: Standard file logging is disabled (LOG_FILE_NAME or LOG_DIR not set). Level: {LOG_LEVEL}")
    ...
    try:
        ...
        if enable_file_logging:
            logger.info(f"Logging to console and file: {os.path.join(LOG_DIR, LOG_FILE_NAME)}")
        else:
        ...
        logging.getLogger(__name__).error("Fell back to basicConfig due to dictConfig error.", exc_info=True)
    ...
    for lib_name, lib_level in libraries_to_quieten.items():
        ...
        if lib_logger.level < lib_level or lib_logger.level == 0: # only change if current level is more verbose or not set
             # Check if specific logger config was applied by dictConfig
            if not (lib_name in config_to_use.get('loggers', {}) and config_to_use['loggers'][lib_name].get('level')):
