...
FALLBACK_FITNESS_WEIGHTS = DEFAULT_EVALUATOR_SETTINGS.get("fitness_score_weights",
...
class ResultsEvaluatorAgent(BaseAgent):
    ...
    def __init__(self, message_bus=None, settings: Optional[Dict] = None, knowledge_file_path: Optional[str] = None): # Modified signature
        ...
        _knowledge_file = self.settings.get("knowledge_file_path", knowledge_file_path) # Direct param can override settings' kfp
        if _knowledge_file:
        ...
        logger.info(f"Agent '{self.agent_id}' initialized with LLM provider: {self.llm_provider}, Evaluation model: {self.evaluation_llm_model}, Knowledge: {self.knowledge_file_path}")
